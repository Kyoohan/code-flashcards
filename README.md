...
- **최근 추가한 카드만 학습** — 한 번에 추가한 카드(노트북·JSON 가져오기, 편집기 세션)가 한 묶음이 됩니다. 묶음 목록에서 최근 k개를 골라 그 카드들만 학습
...
4. 채점기가 틀렸다고 했지만 사실 맞는 답이라면(동치인 코드, 정답 키 오타 등) **정답으로 인정** 버튼을 누르세요. 빈칸 모드는 오답 안내 줄에 있고, 카드가 '일부 오답'으로 끝난 뒤에도 피드백 아래 버튼으로 카드 전체를 정답 처리할 수 있습니다. 전체 코드 모드는 비교 결과 아래에 있습니다. 통계와 라운드 기록이 정답으로 정정되고 바로 다음 문제로 넘어갑니다.
5. 모르겠으면 빈칸을 **비운 채 `Enter`**, 또는 틀린 답을 **고치지 않고 그대로 한 번 더 `Enter`** — 그 칸의 정답이 공개되고 오답으로 기록됩니다. 전체 코드 모드에서는 빈 채로 확인하거나 같은 답을 그대로 다시 확인하면 됩니다.

진행 상황은 자동 저장되며, 탭을 닫거나 새로고침해도 **풀던 자리에서 그대로 이어집니다.** 처음부터 다시 하려면 **초기화**를 누르세요.

### 최근 추가한 카드만 학습하기

카드는 **추가한 단위로 묶음**이 됩니다 — 노트북 가져오기 한 번, JSON 붙여넣기 한 번, 편집기를 열어 연달아 만든 카드들이 각각 한 묶음입니다. 왼쪽 **최근 추가한 카드만 학습**을 누르면 묶음 목록이 최신순으로 뜨고, "최근 N개"를 입력하거나 직접 체크해서 고른 묶음의 카드만 학습합니다. 학습 중 다른 묶음의 카드를 추가해도 현재 범위에 끼어들지 않으며, 새로고침해도 범위가 유지됩니다. **전체 카드로 돌아가기**로 해제합니다.
...
편집기에는 저장 버튼이 없고 **자동 저장**됩니다. 빈칸 지정·해제·별표는 즉시, 설명이나 코드 타이핑은 잠깐 멈추면 저장되며, 하단 상태 줄에 저장 시각이 표시됩니다. 새 카드는 설명과 코드가 모두 채워지는 순간 만들어지고, 한쪽만 적힌 채로 닫으면 저장되지 않는다고 알려줍니다.
...
- **403이 뜨면**: 상태 줄에 원인이 표시됩니다. `요청 한도`면 GitHub의 쓰기 한도(시간당 500회)에 잠시 걸린 것으로, 표시된 시간 뒤 자동으로 다시 올라갑니다. `gist 권한이 없습니다`면 토큰을 classic 타입으로 `gist` 권한을 체크해 다시 만들어 연결하세요.
